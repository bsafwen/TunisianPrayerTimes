#!/usr/bin/env python3
"""
Repair GGML model file v3 — fixes:
  1. Mel filter transpose (201,80) -> (80,201)
  2. Missing token count prefix
  3. Wrong tokenizer encoding
  4. Broken tensor name mapping
  5. Unwanted 32-byte alignment padding
  6. Removes decoder.proj.weight (whisper.cpp uses weight-tying, doesn't expect it)
"""
import struct
import sys
import numpy as np
...
def repair(input_path, output_path):
    ...
    mel_hf = np.array(proc.feature_extractor.mel_filters, dtype=np.float32)
    mel_correct = mel_hf.T  # (80, 201)
    ...
    with open(input_path, "rb") as fin:
        ...
        while True:
            ...
            # Skip decoder.proj.weight — whisper.cpp uses weight-tying
            if fixed_name == "decoder.proj.weight":
                print(f"  SKIPPING: {name} -> {fixed_name}")
                skipped += 1
                continue

            tensors.append({"n_dims": n_dims, "name": fixed_name, "ftype": ftype, "dims": dims, "data": data})

        print(f"Read {len(tensors)} tensors, skipped {skipped}")
    ...
    with open(output_path, "wb") as fout:
        ...
        fout.write(struct.pack("i", mel_correct.shape[0]))
        fout.write(struct.pack("i", mel_correct.shape[1]))
